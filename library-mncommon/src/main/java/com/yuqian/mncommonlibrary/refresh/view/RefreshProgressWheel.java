package com.yuqian.mncommonlibrary.refresh.view;
 ...
 */
public class RefreshProgressWheel extends View {
  private static final String TAG = RefreshProgressWheel.class.getSimpleName();
   ...
   */
  public RefreshProgressWheel(Context context, AttributeSet attrs) {
    super(context, attrs);

    parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.RefreshProgressWheel));
   ...
   * The constructor for the ProgressWheel
   */
  public RefreshProgressWheel(Context context) {
    super(context);
    setAnimationEnabled();
  }
  ...
  private void parseAttributes(TypedArray a) {
    ...
    circleRadius =
        (int) a.getDimension(R.styleable.RefreshProgressWheel_refresh_circleRadius, circleRadius);

    fillRadius = a.getBoolean(R.styleable.RefreshProgressWheel_refresh_fillRadius, false);

    barWidth = (int) a.getDimension(R.styleable.RefreshProgressWheel_refresh_barWidth, barWidth);

    rimWidth = (int) a.getDimension(R.styleable.RefreshProgressWheel_refresh_rimWidth, rimWidth);

    float baseSpinSpeed =
        a.getFloat(R.styleable.RefreshProgressWheel_refresh_spinSpeed, spinSpeed / 360.0f);
    spinSpeed = baseSpinSpeed * 360;

    barSpinCycleTime =
        a.getInt(R.styleable.RefreshProgressWheel_refresh_barSpinCycleTime, (int) barSpinCycleTime);

    barColor = a.getColor(R.styleable.RefreshProgressWheel_refresh_barColor, barColor);

    rimColor = a.getColor(R.styleable.RefreshProgressWheel_refresh_rimColor, rimColor);

    linearProgress = a.getBoolean(R.styleable.RefreshProgressWheel_refresh_linearProgress, false);

    if (a.getBoolean(R.styleable.RefreshProgressWheel_refresh_progressIndeterminate, false)) {
      spin();
    }
